        ...
        function CreateTable(Object, Table, TableID, page) {
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    // json = json.rows;
                    var total = json.resultData.total;
                    json = json.resultData.list;
                    if (json != null) {
                ...
                for (var r = 0; r < json.length; r++) { /*遍历行*/
                    ...
                    for (var c = 0; c < columns.length; c++) { /*遍历列*/
                        ...
                        if (!empty(img) && img) { /*这一列为图片列*/
                            ...
                            //判断是否自定义格式
                            var custom = Object.columns[c]["custom"];
                            var ColumnContentTemp = ColumnContent;
                            if(!empty(custom) && custom){
                                ColumnContentTemp = custom(ColumnContentTemp, json[r]);
                                var pattern = /<\/?[a-zA-Z]+(\s+[a-zA-Z]+=".*")*>/g;
                                if(empty(ColumnContentTemp) || !ColumnContentTemp){
                                }else if(ColumnContentTemp.match(pattern)){
                                    ColumnContent = ColumnContentTemp;
                                    ColumnTitle = $(ColumnContentTemp).text();
                                }else{
                                    ColumnContent = ColumnContentTemp;
                                    ColumnTitle = ColumnContentTemp;
                                }
                            }
                            if(empty(ColumnContent) || !ColumnContent){
                                ColumnContent = '';
                            }
                        ...
                        for (var r = 0; r < jsonCount; r++) { /*遍历行*/
                            ...
                            for (var c = 0; c < columns.length; c++) { /*遍历列*/
                                ...
                                if (table_td.find("img").length > 0) { /*为图片列*/
                                    ...
                                    table_td.attr("title", ColumnTitle);
                                }
                            }
                        }
                        var totalPage = Math.ceil(total / pageCapacity);
