...
public class StringUtils{
	...
	/**
	 * java生成随机数字和字母[纯小写]组合
	 * @param length [生成随机数的长度]
	 * @return
	 */
	public static String getCharAndNumrLowerCase(int length) {
		String val = "";
	...
	public static String getCharAndNumr(int length) {
		...
		return DateUtils.format(new Date(), "yyyyMMdd") + "-" + val;
	}
			...
			for (Object json : arr) {
				if(json != null && json instanceof JSONObject){
					...
					if(json != null && json.toString().length() > 0){
						result.add(JSONObject.parseObject(json.toString()));
					}
				}
			}
			return result;
